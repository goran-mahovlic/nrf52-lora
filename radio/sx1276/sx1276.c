 ...
 * Tx and Rx timers
 */
TIMER_DEF(TxTimeoutTimer);
TIMER_DEF(RxTimeoutTimer);
TIMER_DEF(RxTimeoutSyncWord);
...
void SX1276Init( RadioEvents_t *events )
{
    ...
    uint8_t regVersion = SX1276Read(0x42);
    NRF_LOG_DEBUG("version %x\r\n", regVersion);
...
{
    assert_param( ( SX1276.Spi.initialized == true) );
    ...
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                if( SX1276.DIO4.initialized == false )
                {
